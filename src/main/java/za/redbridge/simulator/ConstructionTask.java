                            ...
                            if (resource.pushedByMaxRobots() || neighbour.pushedByMaxRobots()) {
                                ...
                                if (((schema.checkConfig(schemaNumber, resource.getType(), resAdjacentList)/resource.getNumSidesConnectedTo()) == 1)) {
                                    if (discr.canBeConnected(resource, neighbour, i)) {
                                        ...
                                        czs.add(new ConstructionZone(maxSteps, numCZsStarted));
                                    ...
                                    if (resource.pushedByMaxRobots() || neighbour.pushedByMaxRobots()) {
                                        if ((schema.checkConfig(schemaNumber, resource.getType(), resAdjacentList)/resource.getNumSidesConnectedTo()) == 1) {
                                            if (discr.canBeConnected(resource, neighbour, i)) {
                                                ...
                                                numCZsStarted++;
                                                czs.add(new ConstructionZone(maxSteps, numCZsStarted));
                                                czs.get(numCZsStarted-1).startConstructionZone(resource, neighbour);
    ...
    public void updateCZs() {
        if (czs.size() > 0) {
            ...
            for (ResourceObject res : overallConstructionOrder) {
                ...
                // System.out.println("resCZNum = " + resCZNum);
            ...
            if (generatedTraversals.size() > 0) {
                ...
                // numCZsStarted = czNum-1;
                numCZsStarted = czNum;

                // System.out.println("ORGINAL\t\t\t\tNEW");
                // for (int i = 0; i < newConstructionOrder.size(); i++) {
                //     System.out.println(overallConstructionOrder.get(i) + " " + overallConstructionOrder.get(i).getCzNumber() + "  " + newConstructionOrder.get(i) + " " + newConstructionOrder.get(i).getCzNumber());
                // }
            }
